...
//  * 2. Shards Distribution to Rejuve => The platform
//  *
//  * Phase 2:
...
//     mapping(uint => bool) private _initialDistributionStatus;
...
//      * 2. Every single product has two token types (1155) 1. Locked 2. Tradable
...
//     /**
//      * @dev returns unique URI for unique ID type (locked, tradable)
//      */
...
//      * Remaining shards to Tradable type
//      */
...
//         amounts[1] = amount; // tradable at 1
//         return amounts;
//     }
...
//         address[] memory initialContributors = new address[](initialDataLength); // to store all initial contributors
//         uint256[] memory initialContributorShards = new uint256[](initialDataLength); // to store all initial contributors shards
...
//      *  - Mint 50% shards for Type 0 (Locked) & 50% for Type 1 (Tradable) using BatchMint 1155
//      *
...
//         uint256 labCredit,
//         uint256 initialDataLength,
//         // uint256[] memory initialContributorShards, 
...
//     {
//         ShardConfig storage config = productToShardsConfig[productUID];
//         bytes[] memory productDataHashes = _productNFT.getProductToData(productUID);
//         //uint256 initialDataLength = _productNFT.getInitialDataLength(productUID); 
//         //address[] memory initialContributors = new address[](initialDataLength); // to store all initial contributors
//         //uint256[] memory initialContributorShards = new uint256[](initialDataLength); // to store all initial contributors shards
...
//             uint256[] memory amounts = _setAmount(shardAmount); // 50% should go to locked type & 50% to tradable
//             _mintBatch(
//                 dataOwner,
...
//             //initialContributors[i] = dataOwner;
//             //initialContributorShards[i] = shardAmount;     
//         }

//         //------------- Lab shards --------------

//         uint256 _labShardAmount = _labShards(
//             productUID,
//             lab,
//             labCredit,
//             totalCredits,
...
//         //initialContributors[initialDataLength] = lab;
//         //initialContributorShards[initialDataLength] = _labShardAmount;
...
//         initialContributors[initialContributors.length] = rejuve;
//         //_initialDataOwners.push(rejuve);
//         initialContributorShards[initialContributors.length] = amount;
...
//         uint256 dataHashesLength = productDataHashes.length;
